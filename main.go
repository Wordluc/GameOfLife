...
func main() {
	...
	var pickedPlayable []*core.Character
	w.AddNation(0, map[core.Resource]float32{core.FOOD: 1000})
	for !rl.WindowShouldClose() {
		...
		if rl.IsMouseButtonPressed(rl.MouseLeftButton) {
			if pickedPlayable == nil {
				pickedPlayable = w.GetCharactersAt(getPosMouse(), nil)
			} else {
				w.SetPathCharactersTo(getPosMouse(), pickedPlayable...)
				pickedPlayable = nil
			}
		}
